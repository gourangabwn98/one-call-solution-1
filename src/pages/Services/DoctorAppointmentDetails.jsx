...
const DoctorAppointmentDetails = () => {
  ...
  const item = items.find((m) => m.id === parseInt(doctorId));
  const relatedItems = items
    .filter((m) => m.id !== parseInt(doctorId))
    .slice(0, 3);

  const handleAddToOrder = () => {
    navigate("/services/doctor-appointment", { state: { details: item.name } });
  };
    ...
    return (
      ...
      >
        <div className="container mx-auto px-6 sm:px-8 md:px-12 text-center">
          <h2 className="text-3xl font-semibold">Doctor Not Found</h2>
          <Link
            to="/services/doctor-appointment"
    ...
    >
      <div className="container mx-auto px-6 sm:px-8 md:px-12">
        ...
        >
          ...
          <p className="text-gray-200 mb-4">Specialty: {item.specialty}</p>
          <motion.button
            whileHover={{
              scale: 1.05,
            ...
            {relatedItems.map((related, index) => (
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() =>
                    navigate(`/services/doctor-appointment/${related.id}`)
                  }
